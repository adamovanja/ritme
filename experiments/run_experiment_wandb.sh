...
if [[ ! -f data_splits_wandb/train_val.pkl ]]; then
  ritme split-train-test \
    data_splits_wandb data/movpic_metadata.tsv data/movpic_table.qza \
    --seed 12
fi

# run find-best-model-config only if logs folder empty
if [[ -z "$(find trials_wandb -maxdepth 1 -mindepth 1 -print -quit)" ]]; then
