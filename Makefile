...
dev: all
	pip install pre-commit parameterized ruff black pytest pytest-cov flake8 versioneer mypy types-PyYAML ipykernel $(shell python get_requirements.py ci/recipe/meta.yaml pip)
	pip install -e .
	pre-commit install
