...
| task_type             | Either `"regression"` (default) or `"classification"`. Determines which models, metrics and visualizations are used. When absent, defaults to `"regression"`.                                                                                                                                                                     |
...
`evaluate_tuned_models` takes the best trained models per model type from `find_best_model_config` and evaluates them on both the full training set and a held-out test set. For **regression**, it returns RMSE, R², and Pearson correlation per model/split with true vs. predicted scatter plots. For **classification**, it returns accuracy, balanced accuracy, weighted F1, and Cohen's kappa per model/split with confusion matrix heatmaps. The task type is read from the experiment config (`task_type` field, defaulting to `"regression"`).
