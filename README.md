...
| task_type             | Either `"regression"` (default) or `"classification"`. Determines which models, tuning objective (`rmse_val` / `roc_auc_macro_ovr_val`), and visualizations are used. When absent, defaults to `"regression"`.                                                                                                                                                                     |
| target                | Column name of the target variable in the metadata (numeric for regression, categorical for classification).                                                                                                                                                                                                                                               |
| ls_model_types        | List of model types to explore sequentially. **Regression**: `"linreg"`, `"trac"`, `"xgb"`, `"nn_reg"`, `"nn_class"`, `"nn_corn"`, `"rf"`. **Classification**: `"logreg"`, `"rf_class"`, `"xgb_class"`, `"nn_class"`. Model types must be compatible with the chosen `task_type`. `nn_class` is dual-task and may appear in either list. `nn_corn` is ordinal regression — see `nn_corn_max_levels`.                                                               |
| nn_corn_max_levels    | Optional: upper bound on the number of distinct rounded target levels accepted by `nn_corn`. Defaults to `20`. CORN's output head is `n_levels - 1`, so a wide-ranging continuous target degenerates into a giant ordinal classifier with no extrapolation; this cap rejects such runs up-front. Must be an `int` >= `2`. Use `nn_reg` / `xgb` / `linreg` for continuous regression targets, or raise the cap explicitly if your target is genuinely ordinal with more than 20 levels.                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| max_trial_failure_rate | Optional: maximum tolerated fraction of errored trials (`num_errors / num_trials`) before the search aborts. Defaults to `0.005` (0.5%). At or below the threshold, a warning is emitted listing the failed-trial count and unique Ray Tune error classes, and the campaign proceeds with the surviving trials; above the threshold the search raises `RuntimeError`. Set to `0.0` to restore the strict all-or-nothing abort behavior. |
| seed_data             | Seed for data-related random operations.                                                                                                                                                                                                                                                                                                                   |
| seed_model            | Seed for model-related random operations.                                                                                                                                                                                                                                                                                                                  |                                                                                                                                                                                                                                                                                                      |
| k_folds               | Optional: number of folds for K-fold cross-validation performed inside each trainable. Defaults to 5 and is adaptively capped by the number of independent units (groups, if `group_by_column` is set) and the smallest stratum size. Set `k_folds: 1` for single-split behavior. Each trial reports the mean, standard deviation and standard error of the tuning metric across folds, and `find_best_model_config` then selects the best configuration using a one-standard-error rule.                                                                                                                                                                                                                                              |
...
`evaluate_tuned_models` takes the best trained models per model type from `find_best_model_config` and evaluates them on both the full training set and a held-out test set. For **regression**, it returns RMSE, R², and Pearson correlation per model/split with true vs. predicted scatter plots. For **classification**, it returns `roc_auc_macro_ovr`, `log_loss`, `f1_macro`, `balanced_accuracy`, and MCC per model/split with confusion matrix heatmaps. The task type is read from the experiment config (`task_type` field, defaulting to `"regression"`).
