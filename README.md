...
| ls_model_types        | List of model types to explore sequentially. **Regression**: `"linreg"`, `"trac"`, `"xgb"`, `"nn_reg"`, `"nn_class"`, `"nn_corn"`, `"rf"`. **Classification**: `"logreg"`, `"rf_class"`, `"xgb_class"`, `"nn_class"`, `"nn_corn"`. Model types must be compatible with the chosen `task_type`.                                                               |
...
| k_folds               | Optional: number of folds for K-fold cross-validation performed inside each trainable. Defaults to 5 and is adaptively capped by the number of independent units (groups, if `group_by_column` is set) and the smallest stratum size. Set `k_folds: 1` as the explicit escape hatch for the original single-split behavior. Currently five trainables run true K-fold (`linreg`, `logreg`, `rf`, `rf_class`, `trac`); the others (`xgb`, `xgb_class`, `nn_reg`, `nn_class`, `nn_corn`) accept `k_folds` for signature parity but use a single internal split, emitting a warning when combined with `k_folds > 1`. When K-fold is active, each trial reports the mean, standard deviation and standard error of the tuning metric across folds, and `find_best_model_config` then selects the best configuration using a one-standard-error rule.                                                                                                                                                                                                                                              |
